...
async def test_wrapper_nested_wrappers(caplog):
    ...
    # assert (await read_dir(sd1)).keys() == set()

    # ssd1 = await sd1.create_dir("subsubfolder1")
    ...
    # await ssd1.put_content([vfs.text_file("file3.txt", "abcd")])
    ...
    # assert (await vfs.dir_content_read_dict(await sd1.get_dir_content())).keys() == {
    #     ssd1.name,
    # }
...
async def test_simple1(fixtures: Fixtures):
    ...
    async def test():
        ...
        assert await ctx.listdir_set("/source1/zips") == {
            f"1_{fname1}",
            f"2_{fname2}",
        }

        assert await ctx.listdir_set("/source1/zips", f"1_{fname1}") == set(fls1.keys())

        assert await ctx.listdir_set("/source1/zips", f"2_{fname2}") == set(fls2.keys())
